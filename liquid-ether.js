/* ============================================================
   LiquidEther — vanilla JS port (THREE.js r128+ required)
   Full-screen background fluid simulation.
   ============================================================ */
(function () {
  'use strict';
  ...
  function init() {
    if (typeof THREE === 'undefined') {
      console.warn('LiquidEther: THREE.js not loaded');
      return;
    }
    ...
    var paletteTex = makePaletteTexture(OPTS.colors);
    var bgVec4 = new THREE.Vector4(0, 0, 0, 0); // transparent background
    ...
    MouseClass.prototype.init = function (container) {
      ...
      var win = (this.docTarget && this.docTarget.defaultView) || window;
    ...
    AutoDriver.prototype.update = function () {
      ...
      if (this.rampDurationMs > 0) {
        var tp = Math.min(1, (now - this.activationTime) / this.rampDurationMs);
        ramp = tp * tp * (3 - 2 * tp);
      }
    ...
    ExternalForce.prototype.update = function (props) {
      var fx  = (Mouse.diff.x / 2) * props.mouse_force;
      var fy  = (Mouse.diff.y / 2) * props.mouse_force;
      var csx = props.cursor_size * props.cellScale.x;
      var csy = props.cursor_size * props.cellScale.y;
      var cx  = Math.min(Math.max(Mouse.coords.x, -1 + csx + props.cellScale.x * 2), 1 - csx - props.cellScale.x * 2);
      var cy  = Math.min(Math.max(Mouse.coords.y, -1 + csy + props.cellScale.y * 2), 1 - csy - props.cellScale.y * 2);
    ...
    Simulation.prototype._initSim = function () {
      this.calcSize();
      this.createAllFBO();
    ...
    /* ── Output ── */
    function Output() { this._initOutput(); }
    Output.prototype._initOutput = function () {
    ...
    var ro = new ResizeObserver(function () {
      ...
      resizeRafId = requestAnimationFrame(function () { webgl.resize(); });
    });
    ro.observe(bgEl);
    ...
    document.addEventListener('visibilitychange', function () {
      ...
      else if (isVisibleRef.current) webgl.start();
    });
  }
